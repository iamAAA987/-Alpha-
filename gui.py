...
class ConfigGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        notebook.add(frame_api_email, text="接口与邮箱")
    ...
    def _build_account_tab(self, parent):
        ...
        # 右侧预览列表（直观展示）
        preview_frame = ttk.LabelFrame(parent, text="已配置 Nitter 源（只读预览）")
        preview_frame.grid(row=2, column=2, padx=10, pady=8, sticky=tk.NW)
        self.lst_nitter = tk.Listbox(preview_frame, width=50, height=12)
        self.lst_nitter.pack(side=tk.TOP, padx=5, pady=5)

        btns_frame = ttk.Frame(preview_frame)
        btns_frame.pack(fill=tk.X, padx=5, pady=(0, 6))
        ttk.Button(btns_frame, text="从文本刷新预览", command=self._refresh_nitter_preview).pack(side=tk.LEFT)

        # 初次填充预览
        self._refresh_nitter_preview()
    ...
    def _refresh_nitter_preview(self):
        content = self.txt_nitter.get('1.0', tk.END)
        lines = [ln.strip() for ln in content.splitlines() if ln.strip()]
        self.lst_nitter.delete(0, tk.END)
        for ln in lines:
            self.lst_nitter.insert(tk.END, ln)
    ...
    def _required_config_problems(self) -> list[str]:
        ...
        if not email:
            problems.append("未找到 Email 配置段")
        else:
            if not email.get('sender_email') or not email.get('sender_password'):
                problems.append("发件人邮箱或授权码未设置")
            if not email.get('receiver_email'):
                problems.append("收件人邮箱未设置")
            try:
                int(email.get('smtp_port', '465'))
            except ValueError:
